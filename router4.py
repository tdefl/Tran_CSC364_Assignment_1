...
def start_server():
    ...
    try:
        ...
        forwarding_table_path = "input/router_4_table.csv"
...
def processing_thread(connection, ip, port, forwarding_table_with_range, default_gateway_port, max_buffer_size=5120):
    ...
    # router5_socket = create_socket("127.0.0.1", 8003)
    # router6_socket = create_socket("127.0.0.1", 8004)
...
#           # 7. Store the source IP, destination IP, payload, and TTL.
    ...
    connection.close()
    print("Connections closed by router 4")
